...
export default function SuggestionEntry(props: ISuggestionsHistory): JSX.Element {
  ...
  const getVotesForSuggestion = async (suggestionId: number) => {
    try {
      ...
      const response = await fetch(apiBaseURL + "/votes/" + suggestionId.toString())
  ...
  return (
    <div className="individual-suggestion">
      ...
      <p>{votesCount} votes. Posted at: {props.suggestion.time.slice(0, 10)} {props.suggestion.time.slice(11, 16)} by {giveName()}</p>
      {(props.username === "admin") && <button onClick={()=> handleClick("user")}>{buttonName("user")}</button>}
      <button onClick={()=> handleClick(props.username)}>{buttonName(props.username)}</button>
    </div>
  );
}
